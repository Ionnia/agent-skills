...
const commands = {
  ...
  "add-selfcheck"(flags) { req(flags.topic, "--topic"); say("add-selfcheck", emitBlock(flags, flags.topic, { type: "selfcheck", items: [] }, flags.pos)); },
  "add-selfcheck-item"(flags, pos) {
    const bid = pos[0]; if (!bid) fail("add-selfcheck-item needs <block-id>");
