...
**Trigger:** the skill is invoked with the word `max` after the skill name — e.g. `vue-review max`, or any invocation whose argument is (or contains) `max`. When triggered, follow this Max Mode process **instead of** doing the review yourself. Otherwise, ignore this section and run the normal single-agent process below.
